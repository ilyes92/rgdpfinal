...
class CameraScreen extends React.Component {
  ...
  this.setState({onPictureLoad: true})
...
///////////////////////////////////////////////////////// envoi au backend
    ...
    //http://10.2.1.19:3000/upload
    var ctx = this;
    await fetch("http://10.2.1.19:3000/upload", {
      method: 'post',
      body: data
       ...
       .then(function(data) {
         ctx.setState({onPictureLoad: false})
         // console.log("retour json api microsoft azur ", data);
         //console.log("data", data);
         // passer le tableau coincide dans redux
         ctx.props.scans(data.coincide);
         // renvoyer vers Scan
         ctx.props.navigation.navigate('Scan')
...
////////////////////  suppression du tag sur image tag sur image
  render() {
    const cameraScreenContent = this.state.permissionsGranted
      ? this.renderCamera()
      : this.renderNoPermissions();
    const content = this.state.showGallery ? this.renderGallery() : cameraScreenContent;
    if(this.state.onPictureLoad) {
      return (
        <View style={{flex:1,justifyContent:"center", alignItems:"center"}}>
          <ActivityIndicator size="large"/>
        <Text>Scanner en cours...Veuillez patienter svp !</Text>
        </View>
      )
    }
    return(
...
function mapDispatchToProps(dispatch) {
 return {
   scans: function(matching){
     dispatch({type: 'setScan', scans:matching })
   }
 }
}
